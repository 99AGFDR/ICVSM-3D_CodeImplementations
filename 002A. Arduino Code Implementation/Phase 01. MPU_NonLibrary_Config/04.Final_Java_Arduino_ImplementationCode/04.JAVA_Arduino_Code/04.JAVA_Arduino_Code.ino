...
// JAVA loop for sending floats
  float gForceX = accelX / 4096.0;
  float gForceY = accelY / 4096.0; 
  float gForceZ = accelZ / 4096.0;
  
  char gForceX_text[30];
  char gForceY_text[30];
  char gForceZ_text[30];
  ...
  char text[124];
  snprintf(text, 124, "%s,%s,%s,%s", gForceX_text, gForceY_text, gForceZ_text);
  Serial.println(text);
...
{
   // Output of the LCD (Robojax, 2019)
   lcd.setCursor (tc,tr); //
   lcd.print(title);
